...
import { ArrowLeft, Mail, ShieldCheck, AlertCircle, CheckCircle2, Sparkles } from "lucide-react";
      ...
      <div className="w-full max-w-md bg-white/90 backdrop-blur-2xl rounded-[28px] sm:rounded-[36px] p-6 sm:p-10 shadow-[0_20px_50px_-15px_rgba(12,13,17,0.05)] border border-white/90 ring-1 ring-black/[0.03]">
        ...
        <div className="text-center space-y-2 pb-5 border-b border-black/[0.05]">
          ...
          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-[9px] font-mono font-bold uppercase tracking-widest bg-blue-50 text-[#3B7BF6] border border-blue-100/60">
            <Sparkles className="w-2.5 h-2.5" /> Security Recovery
          </span>
          ...
          <p className="text-[11px] sm:text-xs text-[#8E92A2] max-w-xs mx-auto">
            Provide your registered Gmail address to receive security instructions.
          </p>
        </div>
        ...
        {submitted ? (
          <div className="mt-6 text-center space-y-4">
            <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto border border-emerald-200/80">
              <CheckCircle2 className="w-6 h-6" />
            </div>
            <div className="space-y-1">
              <h3 className="text-sm font-serif font-black uppercase tracking-wider text-[#0C0D11]">
                Recovery Dispatched
              </h3>
              <p className="text-xs text-[#8E92A2] leading-relaxed">
                If an account exists under <strong className="text-[#0C0D11]">{email}</strong>, a secure reset link has been dispatched to your inbox.
              </p>
            </div>
            <Link
              href="/login"
              className="inline-flex items-center gap-1.5 text-xs font-mono font-bold uppercase tracking-wider text-[#0C0D11] hover:text-[#3B7BF6] transition-colors pt-2"
            >
        ...
        <div className="mt-5 pt-4 border-t border-black/[0.05] text-center">
          <Link
            href="/login"
            className="inline-flex items-center gap-1.5 text-xs text-[#8E92A2] hover:text-[#0C0D11] font-mono font-bold uppercase tracking-wider transition-colors"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Return to Sign In
          </Link>
        </div>
